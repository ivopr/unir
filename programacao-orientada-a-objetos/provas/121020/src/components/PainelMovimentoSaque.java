...
public class PainelMovimentoSaque extends JPanel {
  private static final long serialVersionUID = 1L;
  private Campo conta, valor;
  private JButton inserir;

  public PainelMovimentoSaque(ArrayList<ContaSimples> listaSimples, ArrayList<ContaEspecial> listaEspecial, ArrayList<Auth> listaAuth) {
          ...
          if(guarda == 0) {
            ...
            try {
              ...
              if(guarda == 1) {
                if(csim.auth(senha)) {
                  if(csim.saque(dv)) {
                    Auth a = new Auth("Saque Conta Simples");
                    listaAuth.add(a);
                    JOptionPane.showMessageDialog(null, "Saque efetuado.", "SUCESSO!", JOptionPane.INFORMATION_MESSAGE);
                  } else {
                    JOptionPane.showMessageDialog(null, "Não foi possível sacar.", "HALT!", JOptionPane.ERROR_MESSAGE);
                  };
                } else {
                ...
                if(cesp.auth(senha)) {
                  if(cesp.saque(dv)) {
                    Auth a = new Auth("Saque Conta Especial");
                    listaAuth.add(a);
                    JOptionPane.showMessageDialog(null, "Saque efetuado.", "SUCESSO!", JOptionPane.INFORMATION_MESSAGE);
                  } else {
                    JOptionPane.showMessageDialog(null, "Não foi possível sacar.", "HALT!", JOptionPane.ERROR_MESSAGE);
